...
describe('RegisterForm Component', () => {
  ...
  it('updates input fields when user types', () => {
    ...
    const userInput = screen.getByLabelText('common.username') as HTMLInputElement;
    const passInput = screen.getByLabelText('common.password') as HTMLInputElement;
    const confirmInput = screen.getByLabelText('auth.confirm-password') as HTMLInputElement;
  ...
  it('shows error and does not call register if passwords mismatch', () => {
    ...
    fireEvent.change(screen.getByLabelText('common.password'), { target: { value: 'password123' } });
    fireEvent.change(screen.getByLabelText('auth.confirm-password'), { target: { value: 'WRONG' } });
  ...
  it('shows error if password is too short', () => {
    ...
    fireEvent.change(screen.getByLabelText('common.username'), { target: { value: 'user' } });
    fireEvent.change(screen.getByLabelText('common.password'), { target: { value: '123' } }); // < 6 chars
    fireEvent.change(screen.getByLabelText('auth.confirm-password'), { target: { value: '123' } });

    fireEvent.click(screen.getByRole('button', { name: 'auth.register' }));

    expect(screen.getByText('auth.insufficient-password-length')).toBeInTheDocument();
    expect(mockRegisterUser).not.toHaveBeenCalled();
  });
  ...
  it('calls registerUser when inputs are valid', () => {
    ...
    fireEvent.change(screen.getByLabelText('common.username'), { target: { value: 'validUser' } });
    fireEvent.change(screen.getByLabelText('common.password'), { target: { value: 'validPass123' } });
    fireEvent.change(screen.getByLabelText('auth.confirm-password'), { target: { value: 'validPass123' } });
